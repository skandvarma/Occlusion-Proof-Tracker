...
private:
    ...
    cv::Ptr<cv::cuda::Filter> d_bilateral_filter;
    cv::Ptr<cv::cuda::CLAHE> d_clahe;
...
private:
    ...
    void initializeCUDA() {
        // d_bilateral_filter = cv::cuda::createBilateralFilter(CV_8UC3, 5, 50, 50);
        d_clahe = cv::cuda::createCLAHE(2.0, cv::Size(8, 8));
    }
    ...
    void processFrame(const cv::Mat& input, cv::Mat& output) {
        ...
        cv::cuda::bilateralFilter(d_frame, d_denoised, 5, 50, 50);

        // Apply CLAHE
        cv::cuda::GpuMat d_ycrcb;
        cv::cuda::cvtColor(d_denoised, d_ycrcb, cv::COLOR_BGR2YCrCb);
        std::vector<cv::cuda::GpuMat> channels(3);
        cv::cuda::split(d_ycrcb, channels);
        
        d_clahe->apply(channels[0], channels[0]);
        
        cv::cuda::merge(channels, d_ycrcb);
        cv::cuda::cvtColor(d_ycrcb, d_denoised, cv::COLOR_YCrCb2BGR);
        
        if (resize.width > 0 && resize.height > 0) {
            cv::cuda::resize(d_denoised, d_resized, resize, 0, 0, cv::INTER_LINEAR);
            d_resized.download(output);
        } else {
            d_denoised.download(output);
        }
    }
...
public:
    ...
    cv::Size getResizeResolution() const { 
        return resize; 
    }
    ...
    void setResizeResolution(const cv::Size& new_size) {
        ...
        if (resize.width > 0 && resize.height > 0) {
            ...
            d_resized.create(resize, CV_8UC3);
        }
    }
...
int main(int argc, char** argv) {
    ...
    const std::string rtsp_url = "rtsp://192.168.29.218:8554/webCamStream";
