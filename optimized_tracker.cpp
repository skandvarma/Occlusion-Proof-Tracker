...
private:
    ...
    std::string createGstreamerPipeline(const std::string& rtsp_url) {
        return "rtspsrc location=" + rtsp_url + 
               ...
               "omxh264dec ! videoconvert ! " +  // Use hardware decoder on RPi
    ...
    void configureVideoSource() {
        ...
        if (is_rtsp) {
            ...
            cap.set(cv::CAP_PROP_FPS, 30);  // Reduced from 60 for stability
        } else {
            cap.set(cv::CAP_PROP_FPS, 25);  // Reduced from 30
        ...
        if (resize.width > 0 && resize.height > 0) {
            resized = cv::Mat(resize, CV_8UC3);
        }
        denoised = cv::Mat(frame_size, CV_8UC3);
        gray = cv::Mat(frame_size, CV_8UC1);
        hsv = cv::Mat(frame_size, CV_8UC3);
    }

    void processFrame(const cv::Mat& input, cv::Mat& output) {
        // Enhanced preprocessing for low resolution
        cv::Mat enhanced;
        
        // Apply bilateral filter instead of Gaussian for better edge preservation
        cv::bilateralFilter(input, enhanced, 5, 50, 50);
        
        // Apply CLAHE for better contrast in low-res
        cv::Mat ycrcb;
        cv::cvtColor(enhanced, ycrcb, cv::COLOR_BGR2YCrCb);
        ...
        if (resize.width > 0 && resize.height > 0) {
            cv::resize(enhanced, output, resize, 0, 0, cv::INTER_LANCZOS4);
        } else {
            enhanced.copyTo(output);
        }
    }
...
public:
    ...
    bool read(cv::Mat& output_frame) {
        ...
        if (ret) {
            ...
            if (ret && !frame.empty()) {
                processFrame(frame, output_frame);
            }
        }
...
private:
    ...
    const int MAX_FEATURES = 500;  // Increased max features (was 500)
    ...
    void storeTemplate(const cv::Mat& frame, const cv::Rect2d& bbox) {
        try {
            ...
            const std::vector<float> scales = {0.7f, 1.0f, 1.3f};
...
private:
    void initializeDetectors() {
        ...
        feature_detector = cv::AKAZE::create(
            ...
            0,  // descriptor size
            3,  // descriptor channels (reduced from 8)
            0.0001f,  // threshold (increased from 0.0002f)
            4,  // octaves (reduced from 8)
            4,  // octave layers
        ...
        cv::Ptr<cv::flann::IndexParams> indexParams = cv::makePtr<cv::flann::LshIndexParams>(
            12,  // table_number
            20,  // key_size
            2    // multi_probe_level
        );
        cv::Ptr<cv::flann::SearchParams> searchParams = cv::makePtr<cv::flann::SearchParams>(64);
...
private:
    ...
    std::unique_ptr<zmq::socket_t> zmq_publisher;
    const std::string zmq_endpoint = "tcp://*:5555";

    std::unique_ptr<zmq::socket_t> zmq_sync_socket;
    const int SYNC_PORT = 5556;      // Separate port for synchronization

    // ZMQ reliability settings
    const int SYNC_TIMEOUT = 1000;  // milliseconds
    const int HEARTBEAT_INTERVAL = 1000;  // milliseconds
    std::chrono::steady_clock::time_point last_heartbeat;
    int message_sequence = 0;
    std::map<int, json> unacked_messages;
    
    // Message reliability configuration
    const int MAX_UNACKED_MESSAGES = 1000;
    const int MESSAGE_TIMEOUT = 2000;  // milliseconds
    const int MAX_RETRIES = 3;

    // Enhanced ZMQ initialization
    void initializeZMQ() {
        try {
            // Initialize main context
            zmq_context = std::make_unique<zmq::context_t>(1);
            
            // Configure publisher with reliability settings
            zmq_publisher = std::make_unique<zmq::socket_t>(*zmq_context, zmq::socket_type::pub);
            zmq_publisher->set(zmq::sockopt::sndhwm, MAX_UNACKED_MESSAGES);
            zmq_publisher->set(zmq::sockopt::linger, 100);
            zmq_publisher->set(zmq::sockopt::tcp_keepalive, 1);
            zmq_publisher->set(zmq::sockopt::tcp_keepalive_idle, 30);
            zmq_publisher->set(zmq::sockopt::tcp_keepalive_intvl, 5);
            zmq_publisher->bind(zmq_endpoint);
            
            // Initialize sync socket for synchronization
            zmq_sync_socket = std::make_unique<zmq::socket_t>(*zmq_context, zmq::socket_type::rep);
            std::string sync_endpoint = "tcp://*:" + std::to_string(SYNC_PORT);
            zmq_sync_socket->bind(sync_endpoint);
            
            // Wait for subscriber synchronization with timeout
            std::cout << "Waiting for subscriber on sync port " << SYNC_PORT << "..." << std::endl;
            
            std::vector<zmq::pollitem_t> items = {
                {static_cast<void*>(*zmq_sync_socket), 0, ZMQ_POLLIN, 0}
            };
            
            if (!zmq::poll(items, std::chrono::milliseconds(SYNC_TIMEOUT))) {
                throw std::runtime_error("Timeout waiting for subscriber");
            }
            
            // Receive sync request
            zmq::message_t sync_msg;
            auto recv_result = zmq_sync_socket->recv(sync_msg, zmq::recv_flags::none);
            if (!recv_result) {
                throw zmq::error_t();
            }
            
            // Send acknowledgment
            auto send_result = zmq_sync_socket->send(zmq::buffer("READY"), zmq::send_flags::none);
            if (!send_result) {
                throw zmq::error_t();
            }
    ...
    const int PUBLISHER_INIT_DELAY = 500; // milliseconds
    bool publisher_initialized = false;
    std::chrono::steady_clock::time_point publish_start_time;

    const int BATCH_SIZE = 1;                    // Number of messages to batch
    const int MAX_BATCH_DELAY_MS = 100;           // Maximum delay before sending batch
    std::vector<json> message_batch;
    std::chrono::steady_clock::time_point last_batch_time;
    
    // Rate limiting settings
    const int MIN_MESSAGE_INTERVAL_MS = 33;      // Minimum time between messages (~60Hz)
    std::chrono::steady_clock::time_point last_message_time;

    // Update the publish_tracking_data method
    void publish_tracking_data(const TrackerInfo& obj) {
        try {
            auto current_time = std::chrono::steady_clock::now();

            // Rate limiting check
            auto time_since_last_message = std::chrono::duration_cast<std::chrono::milliseconds>(
                current_time - last_message_time).count();
            if (time_since_last_message < MIN_MESSAGE_INTERVAL_MS) {
                return;
            }

            // Handle initialization delay
            if (!publisher_initialized) {
                if (publish_start_time.time_since_epoch().count() == 0) {
                    publish_start_time = current_time;
                    return;
                }

                auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                    current_time - publish_start_time).count();
                
                if (elapsed < PUBLISHER_INIT_DELAY) {
                    return;
                }
            ...
            auto heartbeat_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                current_time - last_heartbeat).count();
            if (heartbeat_elapsed >= HEARTBEAT_INTERVAL) {
                send_heartbeat();
                last_heartbeat = current_time;
            }

            // Clean up old unacked messages
            cleanup_unacked_messages();

            // Calculate center coordinates
            float center_x = obj.last_box.x + obj.last_box.width/2.0f;
            float center_y = obj.last_box.y + obj.last_box.height/2.0f;

            // Add this single print statement
            std::cout << "center_x: " << center_x << ", center_y: " << center_y << std::endl;

            // Create tracking data with sequence number
            json tracking_data = {
                {"sequence", ++message_sequence},
                {"id", obj.id},
                {"center_x", center_x},
                {"center_y", center_y},
                {"width", obj.last_box.width},
                {"height", obj.last_box.height},
                {"confidence", obj.confidence},
                {"velocity", {
                    {"x", obj.smoothed_velocity.x},
                    {"y", obj.smoothed_velocity.y}
                }},
            ...
            // Add to batch
            message_batch.push_back(tracking_data);

            // Check if we should send the batch
            auto batch_age = std::chrono::duration_cast<std::chrono::milliseconds>(
                current_time - last_batch_time).count();
            
            if (message_batch.size() >= BATCH_SIZE || batch_age >= MAX_BATCH_DELAY_MS) {
                // Create batch message
                json batch_message = {
                    {"type", "batch"},
                    {"messages", message_batch},
                    {"batch_size", message_batch.size()},
                    {"timestamp", std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::system_clock::now().time_since_epoch()
                    ).count()}
                };
                ...
                unacked_messages[message_sequence] = batch_message;

                try {
                    std::string msg = batch_message.dump();
                    auto result = zmq_publisher->send(zmq::buffer(msg), zmq::send_flags::dontwait);
                    ...
                    if (msg_count % 100 == 0) {
                        std::cout << "Published " << msg_count << " batches. "
                                << "Unacked: " << unacked_messages.size() << ". "
                                << "Last sequence: " << message_sequence << std::endl;
                    }
                } catch (const zmq::error_t& e) {
                    std::cerr << "Failed to send batch: " << e.what() << std::endl;
                    message_batch.clear();
                }
            }
            ...
            std::cerr << "Error publishing tracking data: " << e.what() << std::endl;
        }
    }

    // Add new method to send message batch
    void send_message_batch() {
        if (message_batch.empty()) return;
        ...
        try {
            ...
            if (batch_count % 20 == 0) {  // Reduced logging frequency
                std::cout << "Published batch " << batch_count 
                         << ". Unacked: " << unacked_messages.size() 
                         << ". Last sequence: " << message_sequence << std::endl;
            }

        } catch (const std::exception& e) {
            std::cerr << "Error sending message batch: " << e.what() << std::endl;
            message_batch.clear();  // Clear batch on error
        }
    }
    ...
    void send_heartbeat() {
        ...
        try {
            zmq_publisher->send(zmq::buffer(heartbeat.dump()), zmq::send_flags::dontwait);
        } catch (const std::exception& e) {
            std::cerr << "Error sending heartbeat: " << e.what() << std::endl;
        }
    }

    void cleanup_unacked_messages() {
        auto current_time = std::chrono::steady_clock::now();
        
        for (auto it = unacked_messages.begin(); it != unacked_messages.end();) {
            int sequence = it->first;
            json& msg = it->second;
            
            auto msg_time = std::chrono::milliseconds(msg["timestamp"].get<int64_t>());
            auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                current_time.time_since_epoch() - msg_time
            ).count();
            
            if (elapsed > MESSAGE_TIMEOUT) {
                int retry_count = msg["retry_count"].get<int>();
                
                if (retry_count < MAX_RETRIES) {
                    // Retry sending the message
                    msg["retry_count"] = retry_count + 1;
                    try {
                        zmq_publisher->send(zmq::buffer(msg.dump()), zmq::send_flags::dontwait);
                        ++it;
                    } catch (const std::exception& e) {
                        std::cerr << "Error retrying message: " << e.what() << std::endl;
    ...
    const double VELOCITY_SMOOTHING = 0.8;  // High smoothing factor
    const size_t VELOCITY_HISTORY_SIZE = 10; // Keep 10 frames of velocity history
    ...
    const int MAX_ROI_AREA = 40000;  // Maximum area in pixels (e.g., 200x200)
    const int MAX_ROI_DIMENSION = 250;  // Maximum width or height
    const float MIN_SCALE_FACTOR = 0.25f;  // Minimum scale for very large ROIs
    const int HIST_BINS_H = 36;  // Hue bins
    const int HIST_BINS_S = 36;  // Saturation bins
    const float HIST_RANGE_H[2] = {0, 180};  // Hue range
    const float HIST_RANGE_S[2] = {0, 256};  // Saturation range
    const int frame_width = 640;   // Default frame width
    const int frame_height = 480;  // Default frame height
...
public:
    ObjectTracker() : next_id(0) {
        try {
            // Initialize ZMQ with higher HWM and proper configuration
            zmq_context = std::make_unique<zmq::context_t>(1);
            zmq_publisher = std::make_unique<zmq::socket_t>(*zmq_context, zmq::socket_type::pub);
            
            // Set higher HWM for better throughput
            int hwm = 1000;
            zmq_publisher->set(zmq::sockopt::sndhwm, hwm);
            zmq_publisher->set(zmq::sockopt::linger, 100);
            
            // Bind to endpoint
            zmq_publisher->bind(zmq_endpoint);
            
            std::cout << "ZMQ publisher initialized on " << zmq_endpoint 
                    << " with HWM " << hwm << std::endl;
            std::cout << "Waiting " << PUBLISHER_INIT_DELAY 
                    << "ms before starting publication..." << std::endl;
            ...
            message_batch.reserve(BATCH_SIZE);
            last_heartbeat = std::chrono::steady_clock::now();
        }
        catch (const zmq::error_t& e) {
            std::cerr << "ZMQ initialization failed: " << e.what() << std::endl;
    ...
    ~ObjectTracker() {
        ...
        if (!message_batch.empty()) {
            try {
                ...
                std::cerr << "Error sending final batch: " << e.what() << std::endl;
            }
        }
        ...
        try {
            ...
            std::cerr << "Error during ZMQ cleanup: " << e.what() << std::endl;
        }
        ...
        std::cout << "ObjectTracker cleanup completed" << std::endl;
    }
   
    
    // In the add_new_object method, modify the tracker initialization part:
    ...
    bool add_new_object(const cv::Mat& frame, const cv::Rect& roi) {
        ...
        try {
            ...
            // Optimize ROI size
            cv::Rect optimized_roi = optimizeROISize(roi, frame.size());
            
            // Extract ROI and calculate initial features
            cv::Mat roi_img = frame(optimized_roi);
            cv::Mat initial_features = extractFeaturesOptimized(roi_img);
            ...
            // Create and initialize tracker with optimized parameters
            cv::Ptr<cv::Tracker> tracker = cv::TrackerCSRT::create();
            cv::Rect2d tracking_roi(optimized_roi);
            
            // Modified initialization - init() returns void, so we try-catch instead
            try {
            ...
            // Create new tracker info with optimized parameters
            TrackerInfo new_tracker(next_id++, tracker, kf, initial_features, optimized_roi);
            ...
            std::cout << "Successfully added new object with ID: " << (next_id-1) 
                    << " (Optimized ROI: " << optimized_roi.width << "x" 
                    << optimized_roi.height << ")" << std::endl;
    ...
    void draw_tracked_objects(cv::Mat& frame) {
        ...
        for (const auto& obj : tracked_objects) {
            ...
            const cv::Scalar ACTIVE_COLOR(255, 255, 255);
            ...
            // Publish tracking data via ZMQ
            publish_tracking_data(obj);
        }
    ...
    void updateAppearanceModel(TrackerInfo& obj, const cv::Mat& frame) {
        try {
            cv::Mat curr_features = extractFeaturesOptimized(frame(obj.last_box));
            
            if (!curr_features.empty()) {
                // Update appearance history
                obj.appearance_history.push_back(curr_features.clone());
                ...
                obj.appearance_model = curr_features.clone();
                
                // Occasionally update template features if tracking is stable
                if (obj.lost_count == 0 && metrics.successful_tracks % 30 == 0) {
                    obj.template_features = curr_features.clone();
                }
            }
        }
        catch (const cv::Exception& e) {
            std::cerr << "Error in updateAppearanceModel: " << e.what() << std::endl;
        }
    }
    ...
    void processSuccessfulTrack(TrackerInfo& obj, const cv::Rect& new_box, 
        ...
        try {
            ...
            if (roi.empty()) {
                throw cv::Exception(0, "Empty ROI", "processSuccessfulTrack", __FILE__, __LINE__);
            }

            cv::Mat curr_features = extractFeaturesOptimized(roi);
            if (curr_features.empty()) {
                throw cv::Exception(0, "Failed to extract features", "processSuccessfulTrack", __FILE__, __LINE__);
            }

            // Compare with template features
            double similarity = compareFeatures(curr_features, obj.template_features);
            std::cout << "Appearance similarity: " << similarity << std::endl;

            const double SIMILARITY_THRESHOLD = 0.3;  // Adjust this value as needed
            ...
            updateAppearanceModel(obj, frame);
            obj.lost_count = 0;
            metrics.successful_tracks++;
        }
        catch (const cv::Exception& e) {
    ...
    void clearAll() {
        ...
        // Reset publisher initialization state
        publisher_initialized = false;
        publish_start_time = std::chrono::steady_clock::time_point();
        ...
        // Create JSON object with tracking data
        json tracking_data;
        tracking_data["is_active"] = false;
        tracking_data["center_x"] = frame_width/2.0;    // Send center of frame
        tracking_data["center_y"] = frame_height/2.0;   // Send center of frame
        tracking_data["velocity"]["x"] = 0.0;
        tracking_data["velocity"]["y"] = 0.0;
        tracking_data["timestamp"] = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::system_clock::now().time_since_epoch()
        ).count();
        
        // Publish the data
        std::string msg = tracking_data.dump();
        zmq_publisher->send(zmq::buffer(msg), zmq::send_flags::dontwait);
    }
...
private:
    ...
    // Updated histogram calculation function
    cv::Mat calculateColorHistogram(const cv::Mat& roi) {
        try {
            if (roi.empty()) {
                std::cerr << "Empty ROI provided to calculateColorHistogram" << std::endl;
                return cv::Mat();
            }
            ...
            int channels[] = {0, 1};  // Use H and S channels
            int histSize[] = {HIST_BINS_H, HIST_BINS_S};
            const float* ranges[] = {HIST_RANGE_H, HIST_RANGE_S};
            cv::Mat hist;

            // Calculate 2D histogram
            calcHist(&hsv, 1, channels, cv::Mat(), hist, 2, histSize, ranges);
            ...
            std::cerr << "Error in calculateColorHistogram: " << e.what() << std::endl;
            return cv::Mat();
        }
    }
    ...
    cv::Rect optimizeROISize(const cv::Rect& original_roi, const cv::Size& frame_size) {
        cv::Rect optimized = original_roi;
        
        // Calculate current area
        int area = optimized.area();
        
        // If ROI is too large, scale it down while maintaining aspect ratio
        if (area > MAX_ROI_AREA || 
            optimized.width > MAX_ROI_DIMENSION || 
            optimized.height > MAX_ROI_DIMENSION) {
            
            float scale_area = std::sqrt(static_cast<float>(MAX_ROI_AREA) / area);
            float scale_dim = std::min(
                static_cast<float>(MAX_ROI_DIMENSION) / optimized.width,
                static_cast<float>(MAX_ROI_DIMENSION) / optimized.height
            );
            
            float scale = std::max(MIN_SCALE_FACTOR, 
                                 std::min(scale_area, scale_dim));
            
            // Calculate new dimensions
            int new_width = static_cast<int>(optimized.width * scale);
            int new_height = static_cast<int>(optimized.height * scale);
            
            // Maintain center position
            int center_x = optimized.x + optimized.width / 2;
            int center_y = optimized.y + optimized.height / 2;
            
            optimized.x = center_x - new_width / 2;
            optimized.y = center_y - new_height / 2;
            optimized.width = new_width;
            optimized.height = new_height;
        }
        
        // Ensure ROI stays within frame bounds
        optimized.x = std::max(0, std::min(optimized.x, frame_size.width - optimized.width));
        optimized.y = std::max(0, std::min(optimized.y, frame_size.height - optimized.height));
        
        return optimized;
    }
    
    // Optimized feature extraction for large ROIs
    cv::Mat extractFeaturesOptimized(const cv::Mat& roi) {
        try {
            cv::Mat processed_roi = roi;
            
            // Scale down large ROIs for feature extraction
            if (roi.cols * roi.rows > MAX_ROI_AREA) {
                float scale = std::sqrt(static_cast<float>(MAX_ROI_AREA) / (roi.cols * roi.rows));
                cv::resize(roi, processed_roi, cv::Size(), scale, scale, cv::INTER_AREA);
            }
            
            return calculateColorHistogram(processed_roi);
        }
        catch (const cv::Exception& e) {
            std::cerr << "Error in extractFeaturesOptimized: " << e.what() << std::endl;
            return cv::Mat();
        }
    }

    // Updated feature comparison function
    double compareFeatures(const cv::Mat& features1, const cv::Mat& features2) {
        try {
            if (features1.empty() || features2.empty()) {
                std::cerr << "Empty features provided to compareFeatures" << std::endl;
                return 0.0;
            }

            // Verify histogram sizes match
            if (features1.size() != features2.size()) {
                std::cerr << "Histogram size mismatch in compareFeatures" << std::endl;
                return 0.0;
            }

            // Compare histograms using correlation method
            double similarity = cv::compareHist(features1, features2, cv::HISTCMP_CORREL);
            ...
            std::cerr << "Error in compareFeatures: " << e.what() << std::endl;
            return 0.0;
        }
    }
    
    // Optimized feature matching for large ROIs
    bool matchFeaturesOptimized(const cv::Mat& template_features, 
                              const cv::Mat& current_features,
                              double& similarity) {
        try {
            // Use a more efficient comparison method for large feature sets
            similarity = cv::compareHist(template_features, current_features, 
                                       cv::HISTCMP_CORREL);
            ...
            std::cerr << "Error matching features: " << e.what() << std::endl;
            return false;
        }
    }
    ...
    void drawTrackerInfo(cv::Mat& frame, const TrackerInfo& obj) {
        ...
        cv::circle(frame, center, 4, cv::Scalar(0, 0, 255), -1);  // Red filled circle
        
        // Draw velocity arrow using smoothed velocity
        cv::Point2f arrow_end = center + obj.smoothed_velocity * 5.0;
        ...
        // Optional: Draw center coordinates
        std::string coord_info = cv::format("(%.1f, %.1f)", center.x, center.y);
        cv::putText(frame, coord_info,
...
public:
    ...
    static void draw_selection(cv::Mat& image) {
        if (drawing || roi_selected) {
            cv::rectangle(image, selection, cv::Scalar(255, 255, 255), 2);
            if (roi_selected) {
...
int main(int argc, char** argv) {
    ...
    const size_t MEMORY_CHECK_INTERVAL = 300; // Check memory every 300 frames
    const size_t MAX_MEMORY_MB = 1000;      // 1GB threshold
    ...
    try {
        ...
        std::deque<double> processing_times(30, 0.0);
        ...
        while (true) {
            ...
            if (!paused) {
                ...
                if (!display_frame.empty()) display_frame.release();
                ...
                if (frame_count % MEMORY_CHECK_INTERVAL == 0) {
                        ...
                        if (getrusage(RUSAGE_SELF, &usage) == 0) {
                            size_t memory_mb = usage.ru_maxrss / 1024; // Convert KB to MB
                            std::cout << "Memory usage (MB): " << memory_mb << std::endl;
                            
                            if (memory_mb > MAX_MEMORY_MB) {
                                std::cout << "High memory detected, performing cleanup..." << std::endl;
                                processing_times.clear();
                                processing_times.resize(30, 0.0);
            ...
            switch (key) {
                ...
                case 'r':
                    ...
                    break;
                case 'm': // Manual memory cleanup
                    std::cout << "Manual memory cleanup..." << std::endl;
                    if (!frame.empty()) frame.release();
                    if (!display_frame.empty()) display_frame.release();
                    processing_times.clear();
                    processing_times.resize(30, 0.0);
