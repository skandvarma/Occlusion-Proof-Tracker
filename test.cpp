...
private:
    ...
    void initializeZMQ() {
        try {
            ...
            zmq_publisher = std::make_unique<zmq::socket_t>(*zmq_context, ZMQ_PUB);
            
            // Set socket options using old-style ZMQ
            int hwm = MAX_UNACKED_MESSAGES;
            zmq_setsockopt(zmq_publisher->operator void*(), ZMQ_SNDHWM, &hwm, sizeof(hwm));
            
            int linger = 100;
            zmq_setsockopt(zmq_publisher->operator void*(), ZMQ_LINGER, &linger, sizeof(linger));
            
            int keepalive = 1;
            zmq_setsockopt(zmq_publisher->operator void*(), ZMQ_TCP_KEEPALIVE, &keepalive, sizeof(keepalive));
            
            int keepalive_idle = 30;
            zmq_setsockopt(zmq_publisher->operator void*(), ZMQ_TCP_KEEPALIVE_IDLE, &keepalive_idle, sizeof(keepalive_idle));
            
            int keepalive_intvl = 5;
            zmq_setsockopt(zmq_publisher->operator void*(), ZMQ_TCP_KEEPALIVE_INTVL, &keepalive_intvl, sizeof(keepalive_intvl));
            
            zmq_publisher->bind(zmq_endpoint);
            
            // Initialize sync socket
            zmq_sync_socket = std::make_unique<zmq::socket_t>(*zmq_context, ZMQ_REP);
            ...
            zmq::pollitem_t items[] = {
                { zmq_sync_socket->operator void*(), 0, ZMQ_POLLIN, 0 }
            };
            
            zmq::poll(items, 1, SYNC_TIMEOUT);
            
            // Receive sync request
            zmq::message_t sync_msg;
            zmq_sync_socket->recv(&sync_msg);
            
            // Send acknowledgment
            zmq::message_t ready_msg(5);
            memcpy(ready_msg.data(), "READY", 5);
            zmq_sync_socket->send(ready_msg);
    ...
    void send_message_batch() {
        ...
        try {
            ...
            zmq::message_t message(msg.size());
            memcpy(message.data(), msg.c_str(), msg.size());
            zmq_publisher->send(message, ZMQ_DONTWAIT);
...
public:
    ObjectTracker() : next_id(0) {
        try {
            ...
            // Set socket options using low-level ZMQ API
            int hwm = 1000;
            zmq_setsockopt(zmq_publisher->operator void*(), ZMQ_SNDHWM, &hwm, sizeof(hwm));
    ...
    void clearAll() {
        ...
        tracking_data["center_x"] = frame_width/2.0;
        tracking_data["center_y"] = frame_height/2.0;
...
private:
    ...
    void handleOcclusions(const cv::Mat& frame) {
        ...
        for (const auto& overlap : overlaps) {
            resolveOcclusion(tracked_objects[overlap.first], 
                            tracked_objects[overlap.second], frame);
        }
    }
